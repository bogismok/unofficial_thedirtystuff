...
public class OpiumRightclickedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (entity instanceof Player _player)
			_player.getCooldowns().addCooldown(TheDirtyStuffModItems.OPIUM.get(), 900);
		TheDirtyStuffMod.queueServerWork(600, () -> {
			if ((entity.getCapability(TheDirtyStuffModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new TheDirtyStuffModVariables.PlayerVariables())).opium) {
				if (entity instanceof LivingEntity _entity && !_entity.level.isClientSide())
