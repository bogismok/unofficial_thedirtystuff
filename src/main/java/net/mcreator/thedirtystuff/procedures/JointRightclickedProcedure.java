...
public class JointRightclickedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getItem() == Items.FLINT_AND_STEEL) {
			...
			if (entity instanceof Player _player)
				_player.getCooldowns().addCooldown(TheDirtyStuffModItems.JOINT.get(), 900);
			TheDirtyStuffMod.queueServerWork(600, () -> {
				if ((entity.getCapability(TheDirtyStuffModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new TheDirtyStuffModVariables.PlayerVariables())).joint) {
					if (entity instanceof LivingEntity _entity && !_entity.level.isClientSide())
						_entity.addEffect(new MobEffectInstance(MobEffects.HUNGER, 300, 99));
				}
			});
