	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getItem() == Items.FLINT_AND_STEEL) {
			...
			if (entity instanceof Player _player)
				_player.getCooldowns().addCooldown(TheDirtyStuffModItems.CIGARETTE.get(), 300);
			TheDirtyStuffMod.queueServerWork(200, () -> {
				if ((entity.getCapability(TheDirtyStuffModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new TheDirtyStuffModVariables.PlayerVariables())).cigarette) {
					if (entity instanceof LivingEntity _entity && !_entity.level.isClientSide())
